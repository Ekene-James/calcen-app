...
import { addUser, editUser, handleSnackBar } from '../../store/admin/AdminAction';
import { AdminContext } from '../../store/admin/AdminStore';

export default function EditUser({setinView}) {
  const authCtx = React.useContext(AuthContext);
    const adminCtx = React.useContext(AdminContext);
  const [state, setstate] = React.useState({
    first_name:adminCtx.state.editUser.first_name,
    last_name:adminCtx.state.editUser.last_name,
    room_number:adminCtx.state.editUser.room_name || '',
    display_name:adminCtx.state.editUser.display_name,
    email:adminCtx.state.editUser.email,
    // user_unique_id:adminCtx.state.editUser.unique_id,
    // extension_number:adminCtx.state.editUser.extension_number,
  ...
  const handleAdd = () => {
    ...
    adminCtx.dispatch(editUser(state,adminCtx.dispatch))
  }
