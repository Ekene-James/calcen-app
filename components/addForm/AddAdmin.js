...
import { addAdminUser, handleSnackBar } from '../../store/admin/AdminAction';
...
export default function AddAdmin({setinView}) {
 ...
 const handleAdd = () => {
  if(!state.password || (state.password !== state.confirmPassword)){
      adminCtx.dispatch(handleSnackBar({
        type:'error',
        open:true,
        msg: authCtx.state.isEnglish ?  'Password and Confirm password must be provided and same' : 'パスワードとパスワードの確認が提供され、同じである必要があります'
      }))
        return;
    }
  if(menuTxt.engTxt === 'Choose Role'){
    ...
    adminCtx.dispatch(addAdminUser(data,adminCtx.dispatch))
}
  return (
    ...
    <Grid container spacing={3}>
      ...
      <Grid item xs={12} md={6}>
        ...
        <Grid item sm={6} md={2}>
          <Button variant="contained" color="primary" onClick={handleAdd} disabled={adminCtx.state.loadings.isLoading}>
