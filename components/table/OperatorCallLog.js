...
const TCell = ({row}) => {
 ...
 const date = new Date(row.created_at).toLocaleDateString('en-us', { weekday:"long", year:"numeric", month:"short", day:"numeric"})
  ...
  return (
    <>
          ...
          <TableCell align="left" >{row.from_number}</TableCell>
 ...
 function OperatorCallLog({setinView}) {
  const classes = useStyles();
    ...
    const header = [
      ...
      <span key={2} className={classes.title2}><CustomTxt engTxt='Call From' japTxt='発信元' color='#174A84' type='p' style={{fontSize:'12px',fontWeight:'bold',margin:0}}/></span>,
      <span key={3} className={classes.title2}><CustomTxt engTxt='User Name' japTxt='ユーザ名' color='#174A84' type='p' style={{fontSize:'12px',fontWeight:'bold',margin:0}}/></span>,
      <span key={4} className={classes.title2}><CustomTxt engTxt='Talk Time' japTxt='通話時間' color='#174A84' type='p' style={{fontSize:'12px',fontWeight:'bold',margin:0}}/></span>,
    ...
    const handleClick = (pg) => {
      ...
      adminCtx.dispatch(getCallLogs(adminCtx.dispatch,true,pg))
     };
...
export default OperatorCallLog;